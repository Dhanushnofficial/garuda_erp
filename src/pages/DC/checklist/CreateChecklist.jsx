...
const CreateLOI = () => {
  ...
  const generateLOINumber = async () => {
    try {
      const snapshot = await getDocs(collection(db, "listOfItems"));
      const count = snapshot.docs.length + 1;
      setLoiNumber(`GA/LOI/${getFiscalYear()}/${String(count).padStart(3, "0")}`);
    } catch (error) {
  ...
  const executePrintAndSave = async () => {
    try {
      ...
      // Format the LOI number into a filesystem-safe filename layout
      const safeFileName = loiNumber ? loiNumber.replace(/\//g, "-") : "List-Of-Items";
      ...
      await addDoc(collection(db, "listOfItems"), {
        loiNumber,
        ...formData,
        items,
      ...
      alert("System Registry Confirmed: LOI Manifest successfully committed.");
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 xl:p-8 ml-[250px] font-sans antialiased selection:bg-blue-500 selection:text-white print:m-0 print:p-0 print:bg-white print:ml-0">
        ...
        <div className="mx-auto flex max-w-[1400px] flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-2xl xl:text-3xl font-extrabold text-white tracking-tight">Create List Of Items Checklist</h1>
            </div>
          </div>
          <button
      ...
      <div className="no-print-terminal space-y-6 mb-10 pt-28">
        ...
        {/* PANEL ROW 1: PRIMARY EXPEDITION METRICS */}
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-5">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            ...
            <h3 className="font-bold text-sm text-slate-800 uppercase tracking-wider">Challan Link Parameters</h3>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="space-y-2">
              <label className="block font-bold text-xs text-slate-400 uppercase tracking-wide">Delivery Challan Number</label>
              <input
                type="text"
                name="challanNumber"
                placeholder="GA/DC/26-27/XXX"
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/40">
            ...
            >
              <FaPlus className="text-[10px]" /> Append Item Code
            </button>
          </div>
                    ...
                    <tbody>
                      <tr className="flex divide-x divide-slate-950">
                        <td className="p-3 w-1/2 align-top leading-relaxed space-y-2">
                          <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">Inspection Date:</b> {formatDate(formData.date) || "—"}</p>
                        ...
                        <td className="p-3 w-1/2 align-top leading-relaxed space-y-2" style={{ borderLeft: "1px solid #000" }}>
                          <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">LOI Reference No:</b> <span className="font-mono font-bold text-slate-900">{loiNumber || "Generating..."}</span></p>
                          <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">System Status:</b> <span className="text-xs uppercase tracking-wider font-bold bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded">Active Audit</span></p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="grid grid-cols-2 gap-8 items-end">
                  ...
                  <div className="space-y-6 pb-2">
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] font-medium text-slate-400 italic">Electronic Loop Validation System</p>
                    </div>
                    <div className="border-t border-slate-950 w-44 pt-0.5 text-xs font-bold text-slate-800 tracking-wide " style={{ borderTop: "1px solid #000 " }}>
