...
const ChecklistPreview = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Packing checklist preview data stream anomaly:", error);
      setLoading(false);
    }
  };
  ...
  const handlePrint = () => {
    ...
    const safeFileName = data?.checklistNumber ? data.checklistNumber.replace(/\//g, "-") : "Packing-Checklist";
  ...
  if (!data) {
    return (
      <div className="min-h-screen bg-slate-50 ml-[250px] flex flex-col justify-center items-center text-slate-500">
        ...
        <p className="text-sm text-slate-400 mb-6">The requested Packing Checklist database profile could not be resolved.</p>
        <button onClick={() => navigate("/dc/packing-history")} className="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow">
          Return to History
        </button>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 p-6 xl:p-8 ml-[250px] font-sans antialiased selection:bg-blue-500 selection:text-white print:m-0 print:p-0 print:bg-white print:ml-0">
        ...
        <div className="no-print-terminal flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 bg-slate-900 rounded-3xl p-6 border border-slate-800 shadow-xl relative overflow-hidden">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Auditing cargo package lines and inspection markers before printing execution.</p>
          </div>
        ...
        <div className="space-y-4 print:p-0 print:m-0">
          ...
          <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700/50 shadow-inner overflow-x-auto flex justify-center print:bg-white print:p-0 print:border-none print:shadow-none">
            ...
            >
              ...
              {/* FLEX BOX WRAPPER DRIVING ABSOLUTE BASE FOOTER POSITIONING */}
              <div className="w-full flex flex-col h-full justify-between min-h-[265mm]">
                      ...
                      <tbody>
                        <tr className="flex divide-x divide-slate-950">
                          <td className="p-3 w-1/2 align-top leading-relaxed space-y-2">
                            <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">Dispatch Date:</b> {data?.dispatchDate || "—"}</p>
                          ...
                          <td className="p-3 w-1/2 align-top leading-relaxed space-y-2" style={{ borderLeft: "1px solid #000" }}>
                            <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">Checklist Reference No:</b> <span className="font-mono font-bold text-slate-900">{data?.checklistNumber || "—"}</span></p>
                            <p className="text-slate-800 font-medium"><b className="text-slate-900 font-bold">System Status:</b> <span className="text-xs uppercase tracking-wider font-bold bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded">Cargo Verified</span></p>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                ...
                {/* CORPORATE VERIFICATION DISPATCH FOOTER MATRIX (LOCKED DOWN AT BOTTOM OF CONTAINER) */}
                <div className="relative z-10 space-y-6 mt-12 print:break-inside-avoid">
                  <div>
                    <table className="w-full border-collapse text-[12px] font-medium border border-slate-950" style={{ border: "1px solid #000" }}>
                      ...
                      <tbody>
                        <tr className="divide-x divide-slate-950 h-20 align-top">
                          <td className="p-2 break-words font-bold text-slate-900" style={{ borderRight: "1px solid #000" }}>{data?.packingPerson || data?.packedBy || "—"}</td>
                          <td className="p-2 break-words font-bold text-slate-900" style={{ borderRight: "1px solid #000" }}>{data?.checkingPerson || data?.inspectedBy || "—"}</td>
                          <td className="p-2 font-mono text-slate-400 italic" style={{ borderRight: "1px solid #000" }}>{data?.salesSignature || "—"}</td>
                          <td className="p-2 font-mono text-slate-400 italic">{data?.customerSignature || "—"}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>

                  {/* FIXED CORE LABELS ALIGNED SECURELY TO THE CORRESPONDING ATTESTATION SEALS */}
                  <div className="flex justify-between items-end pt-1 max-w-full">
                    <div className="space-y-1">
                      <p className="font-bold text-slate-900 text-base uppercase tracking-wide">For Garuda Aerospace.</p>
                      <p className="text-xs font-medium text-slate-400 italic">Secure Internal Logistics System Token</p>
                    </div>
                  ...
                  {/* CORE BASE INFRAFOOTER MAPPED PERFECTLY TO STABILIZE SPATIAL BOUNDARIES */}
                  <div className="border-t border-slate-300 pt-3 flex justify-between text-[11px] font-medium text-slate-500 font-sans tracking-wide leading-relaxed print:mt-10" style={{ borderTop: "1px solid #cbd5e1" }}>
                    <div>
